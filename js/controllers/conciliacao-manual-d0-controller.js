angular.module('trustionPortal').controller('ConciliacaoManualD0Controller', function($scope, $uibModalInstance, relatorio, ExtratoElegivelService, UTF8){
    ...
    function loadPage(){
        ocultarMensagemErro();
        ExtratoElegivelService.listarPorIdRelatorioAnaliticoExtratosElegiveisD0($scope.relatorio.idRelatorioAnalitico).then(
    ...
    $scope.conciliar = function(){
        ...
        if(ExtratosConciliados && ExtratosConciliados.length){   
            ...
            ExtratoElegivelService.conciliarD0($scope.relatorio.idRelatorioAnalitico, ExtratosConciliados).then(function sucessCallback(res){
